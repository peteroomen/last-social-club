...
function CardFace({card,onClick,selected=false,disabled=false,compact:small=false,trump}:{card:Card;onClick?:()=>void;selected?:boolean;disabled?:boolean;compact?:boolean;trump?:Suit}){
 ...
 return onClick?<button className={cls} aria-label={label} aria-pressed={selected} onClick={onClick} aria-disabled={disabled}>{content}</button>:<div className={cls} role="img" aria-label={label}>{content}</div>;
}
...
export default function App(){
 ...
 return <main onPointerDown={unlockSound} onKeyDown={unlockSound} className={`club ${isTable?'table-screen':s.phase==='arrival'||s.phase==='pub'?'room-screen':'summary-screen'} phase-${s.phase} ${s.insight>=3?'after-hours':''}`}>
  <header className="masthead"><div><h1>THE LAST SOCIAL CLUB</h1><span className="version-chip">FIRST SITTING · v0.2</span></div><button className="menu-button" onClick={()=>setModal('menu')} aria-label="Open menu">☰</button></header>
  ...
  <div className="regular-rack" aria-label="Your Regulars">{Array.from({length:5},(_,i)=>{const id=s.regulars[i];return id?<button key={id} className={`regular-card ${feedback?.entries.some(e=>e.label.includes(regular(id).name))?'triggered':''}`} onClick={()=>setModal(`regular:${id}`)}><Portrait id={regular(id).portrait}/><span>{regular(id).short}</span></button>:<div key={i} className="regular-empty" aria-label="Empty Regular slot">+</div>;})}</div>
  {isTable&&<>

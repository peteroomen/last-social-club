...
import {fresh,reduce,aiAction,observation,isPlayer,seatName,SUITS,CONTRACTS,nextBid,canBreakSuit,ledSuit,symbol,cardSuit,cardName,rankName,sortHand,effective,legalCards,bidValue,failCost,trickWinner,pending,type State,type Action,type Card,type ContractSuit,type Suit,type Seat,type Entry} from '../engine/game.ts';
...
export default function App(){
 ...
 const canSelect=!flight&&(s.phase==='play'||s.phase==='kitty')&&ourTurn&&shownSeat===s.turn;
 ...
 const startNew=(same=false)=>{setFeedback(null);setFlight(null);flightLock.current=false;setS(fresh(same?s.seed:seed(),campaign.nights+1));setModal(null);setSelected([]);setInspect(null);};
 ...
 return <main ref={stageRef} onPointerDown={unlockSound} onKeyDown={unlockSound} className={`club ${isTable?'table-screen':s.phase==='arrival'||s.phase==='pub'?'room-screen':'summary-screen'} phase-${s.phase} ${s.insight>=3?'after-hours':''} ${flight?'collecting-trick':''}`}>
  <header className="masthead"><div><h1>THE LAST SOCIAL CLUB</h1><span className="version-chip">FIRST SITTING · v0.4</span></div><div className="header-controls"><button className="fullscreen-button" onClick={toggleFullscreen} aria-label={fullscreen?'Exit fullscreen':'Enter fullscreen'} title={fullscreen?'Exit fullscreen':'Enter fullscreen'}><span aria-hidden="true">{fullscreen?'↙':'⛶'}</span><span>{fullscreen?'Exit':'Full screen'}</span></button><button className="menu-button" onClick={()=>setModal('menu')} aria-label="Open menu">☰</button></div></header>
